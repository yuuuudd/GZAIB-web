...
const TOPIC_MAX_LENGTH = 30;

const EMAIL_PATTERN = /\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/i;
const PHONE_PATTERN = /(?:\+?86[-\s]?)?1[3-9]\d(?:[-\s]?\d){8}\b/;
const WECHAT_PATTERN = /(?:微信(?:号)?|\b(?:wechat|weixin|vx)\b)\s*(?:(?:id|号|is|为|是)\s*)?[:：=]?\s*[@a-z0-9][a-z0-9_-]{3,}/i;
...
export function canCreate(context: ConnectionPolicyContext): PolicyResult {
  ...
  if (context.senderStatus !== "active") return { ok: false, code: "sender_ineligible" };
