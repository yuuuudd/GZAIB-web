...
import { safetyDialogSuccessState } from "./dialog-focus";
...
export function BlockButton({ memberSlug, name = "该成员", onBlocked }: { memberSlug: string; name?: string; onBlocked?(): void }) {
  ...
  return <div className="safety-action">
    {confirming ? <Modal role="alertdialog" labelledBy="block-dialog-title" onDismiss={closeAndRestoreFocus}><div className="connection-dialog">
    ...
    </div></Modal> : <button ref={triggerRef} type="button" onClick={() => setConfirming(true)}>拉黑</button>}
  </div>;
}
