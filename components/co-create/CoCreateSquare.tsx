...
export function CoCreateSquare() {
  ...
  return <>
    ...
    <section className="co-create-flow" aria-labelledby="flow-title"><h2 id="flow-title">让共创真正发生</h2><ol>{["提出想法", "招募伙伴", "组建共创小组", "一起行动", "完成共创", "留下记录"].map((step, index) => <li key={step}><i>{["✦", "◎", "◉", "↗", "✓", "▤"][index]}</i><span>{step}</span></li>)}</ol></section>
  </>;
}
