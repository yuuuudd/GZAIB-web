...
import { CO_CREATE_PARTICIPATION_MODES, CO_CREATE_TYPES, filterCoCreates } from "../../features/co-create/catalog";
...
function scheduleLabel(project: PublicCoCreateProject) {
  return project.timeTbd || !project.startsAt || !project.endsAt ? "时间待定" : `${project.startsAt.replace("T", " ")} – ${project.endsAt.replace("T", " ")}`;
}
...
export function CoCreateSquare({ items: allItems, signedIn, loginHref }: { items: PublicCoCreateProject[]; signedIn: boolean; loginHref: string }) {
  ...
  const [mode, setMode] = useState<(typeof CO_CREATE_PARTICIPATION_MODES)[number] | "全部">("全部");
  const [selected, setSelected] = useState<PublicCoCreateProject>();
  const closeRef = useRef<HTMLButtonElement>(null);
  const items = filterCoCreates(allItems, type, mode);
  const metrics = [
    ["◎", "公开共创", allItems.length, "个"],
    ["●", "招募中", allItems.filter((item) => item.status === "招募中").length, "个"],
    ["✓", "线下开展", allItems.filter((item) => item.participationMode === "线下").length, "个"],
    ["▥", "线上或混合", allItems.filter((item) => item.participationMode !== "线下").length, "个"],
  ] as const;
  useEffect(() => {
    if (!selected) return;
