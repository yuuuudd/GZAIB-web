...
export function ProfileEditor({
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    await request("/api/me/profile", "PATCH", {
      ...
      visibility, ...(avatarKey ? { avatarKey } : {}),
    });
  }

  return <div className="profile-editor-stack">
    <section className="member-hub-card">
      <div className="member-hub-identity">
        <div className="member-hub-avatar" role="img" aria-label={`${profile.nickname ?? "共建者"}的头像`}>
          {/* Owner avatar URLs are normalized by the avatar API. */}
        ...
        <div><p className="section-kicker">我的共创空间</p><h1>{profile.nickname}</h1><p>{published ? "你的光已经亮起，资料正在共建地图中展示。" : "资料目前没有在地图中展示。"}</p>
      ...
      <nav className="member-hub-actions" aria-label="我的共创空间功能"><a href="#edit-profile">编辑公开资料</a><a href="/me/contact-card">私密联系方式</a><a href="/me/connections">我的连接</a><a href="/me/communities">我的社群</a><a href="/me/activities">活动申请</a><a href={`/members/${profile.slug}`}>查看公开主页 ↗</a></nav>
    </section>
    <form className="profile-editor" onSubmit={save}>
      <header id="edit-profile"><div><p className="section-kicker">编辑资料</p><h1>管理我的公开资料</h1></div></header>
      ...
      <button className="application-submit" type="submit" disabled={busy || uploadingAvatar}>{busy ? "正在保存…" : uploadingAvatar ? "请等待头像处理完成…" : "保存资料与公开设置"}</button>
    </form>
