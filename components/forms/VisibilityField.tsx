...
export function VisibilityField({ label, value, onChange }: VisibilityFieldProps) {
  return (
    <label className="visibility-field">
      <span>{label}</span>
      <select value={value} onChange={(event) => onChange(event.target.value as Visibility)} aria-label={`${label}公开范围`}>
