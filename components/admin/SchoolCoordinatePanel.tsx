...
import { AmapLoader, type AmapNamespace } from "../map/AmapLoader";

type School = { id: string; name: string; campus: string; city: string; longitude: number; latitude: number; coordinateStatus: string };
type Candidate = { name: string; city: string; longitude: number; latitude: number };
...
function SchoolSearch({ amap, onSelect }: { amap: AmapNamespace; onSelect: (candidate: Candidate) => void }) {
  const [query, setQuery] = useState(""); const [results, setResults] = useState<Candidate[]>([]); const [message, setMessage] = useState("");
  function search() { const keyword = query.trim(); if (!keyword) return; setMessage("正在搜索高德地图…"); new amap.PlaceSearch({ city: "广东" }).search(keyword, (status, value) => {
    const pois = status === "complete" && value && typeof value === "object" ? ((value as { poiList?: { pois?: unknown[] } }).poiList?.pois ?? []) : [];
    const next = pois.flatMap((poi) => { if (!poi || typeof poi !== "object") return []; const row = poi as { name?: unknown; cityname?: unknown; adname?: unknown; location?: unknown }; const coordinate = locationOf(row.location); return coordinate && typeof row.name === "string" ? [{ name: row.name, city: typeof row.cityname === "string" && row.cityname ? row.cityname : typeof row.adname === "string" ? row.adname : "广州", ...coordinate }] : []; }).slice(0, 8);
    setResults(next); setMessage(next.length ? "请选择一个高德搜索结果" : "没有找到可用地点，可在下方手动填写坐标");
  }); }
  return <section className="school-search"><h2>搜索高德学校地点</h2><p>选择结果后会作为待确认坐标保存；确认后才可用于公开成员地图。</p><div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="例如：中山大学广州校区" aria-label="搜索学校" /><button type="button" className="action-primary" onClick={search}>搜索</button></div>{message ? <p role="status">{message}</p> : null}<ul>{results.map((candidate) => <li key={`${candidate.name}-${candidate.longitude}`}><button type="button" onClick={() => onSelect(candidate)}><strong>{candidate.name}</strong><span>{candidate.city} · {(candidate.longitude / 1_000_000).toFixed(5)}, {(candidate.latitude / 1_000_000).toFixed(5)}</span></button></li>)}</ul></section>;
}
