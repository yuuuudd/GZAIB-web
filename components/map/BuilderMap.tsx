...
export function BuilderMap() {
  ...
  const allCities = useMemo(() => groupSchoolsByCity(schools), [schools]);
  const provinces = useMemo(() => groupSchoolsByProvince(schools), [schools]);
  const cities = useMemo(() => citiesForProvince(provinces, activeProvince), [activeProvince, provinces]);
  ...
  const selectProvince = useCallback((province: string) => {
    ...
    setActiveProvince(normalized);
    setActiveCity(citiesForProvince(provinces, normalized)[0]?.city ?? DEFAULT_CITY);
  ...
  const selectSchool = useCallback((school: DirectorySchool) => {
    setActiveProvince(normalizeProvince(school.province));
    ...
    <section className="builder-map-section" id="map" aria-labelledby="map-title">
      ...
      <div className="map-overview-row">
        <nav className="semantic-level-switcher" aria-label="共建地图层级">
          <button type="button" aria-pressed={level === "city"} onClick={() => selectCity(activeCity)}>{activeCity}</button>
          <button type="button" aria-pressed={level === "province"} onClick={() => changeLevel("province")}>{activeProvince}</button>
        ...
        <div className="map-stats" aria-label="目录统计">
          ...
          <div><span aria-hidden="true">城</span><strong>{allCities.length}</strong><p>座城市</p></div>
        </div>
      </div>
        ...
        <div className="map-live-layout">
          <div className="map-live">
            <AmapLoader>{(state, amap, retry) => state === "ready" && amap
                ? <SemanticMapCanvas amap={amap} provinces={provinces} level={level} activeProvince={activeProvince} activeCity={activeCity} selectedId={selectedId} onLevelChange={changeLevel} onSelectProvince={selectProvince} onSelectCity={selectCity} onSelectSchool={selectSchool} onFailure={() => retry?.()} />
                : state === "failed"
