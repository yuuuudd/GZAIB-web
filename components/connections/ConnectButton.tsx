...
export function ConnectButton({ state, recipientSlug, recipientName, dailyRemaining }: {
  ...
  return <>{dialogOpen ? <ConnectionRequestDialog recipientSlug={recipientSlug} recipientName={recipientName} dailyRemaining={dailyRemaining} onClose={closeAndRestoreFocus} onCreated={() => { setCurrentState("pending"); setDialogOpen(false); }} /> : null}<button ref={triggerRef} className="connection-cta" type="button" onClick={() => setDialogOpen(true)}>想认识 TA <span aria-hidden="true">→</span></button></>;
}
