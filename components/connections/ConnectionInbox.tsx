...
export function ConnectionInbox({ initialBox = "new" }: { initialBox?: Box }) {
  ...
  const load = useCallback(async (target: Box, cursor?: string) => {
    ...
    } catch (cause) { if (version === loadVersion.current) setError(cause instanceof Error ? cause.message : "加载失败"); }
  ...
  return <section className="connection-inbox" aria-labelledby="connection-inbox-title"><header><div><p className="section-kicker">一起认识，一起共创</p><h1 id="connection-inbox-title">我的连接</h1><p>在这里遇见伙伴，让一个想法有新的开始。</p></div><a className="connection-explore" href="/map">去发现共创伙伴 <span aria-hidden="true">↗</span></a></header><p className="connection-privacy-note"><span aria-hidden="true">◇</span> 联系方式仅在双方同意后交换</p><div className="connection-tabs" role="tablist" aria-label="连接请求分类">{(Object.keys(labels) as Box[]).map((value) => <button key={value} role="tab" aria-selected={box === value} type="button" onClick={() => setBox(value)}>{labels[value]}</button>)}</div>{error && !selected ? <p className="connection-error" role="alert">{error} <button type="button" onClick={() => void load(box)}>重试</button></p> : null}{loading ? <p className="connection-empty" role="status">正在加载连接请求…</p> : !error && items.length === 0 ? <div className="connection-empty"><span className="connection-empty-mark" aria-hidden="true">↗</span><h2>{box === "new" ? "下一位共创伙伴，等你认识" : "让连接从一次交流开始"}</h2><p>{box === "new" ? "收到和发出的连接申请，都会出现在这里。" : "双方接受连接后，就能在这里找到彼此。"}</p><a href="/map">去共建地图看看 <span aria-hidden="true">→</span></a></div> : <div className="connection-list">{items.map((item) => <ConnectionCard key={item.request.id} item={item} box={box === "new" ? item.direction ?? "received" : "accepted"} busy={busyId === item.request.id} onView={() => setSelected(item)} onAction={(action) => void resolve(item, action)} />)}</div>}{nextCursor && !loading ? <button className="connection-more" type="button" onClick={() => void load(box, nextCursor)}>加载更多</button> : null}{selected?.counterpart ? <IncomingRequestDialog item={selected} error={error} busy={busyId === selected.request.id} onClose={() => setSelected(undefined)} onAction={(action) => void resolve(selected, action)} /> : null}</section>;
}

function IncomingRequestDialog({ item, busy, error, onClose, onAction }: { item: InboxItem; busy: boolean; error: string; onClose(): void; onAction(action: "accept" | "decline"): void }) {
  const member = item.counterpart!;
  return <Modal labelledBy="incoming-member-name" onDismiss={onClose}><section className="connection-dialog connection-profile-dialog"><button className="connection-dialog-close" type="button" onClick={onClose} aria-label="关闭资料卡">×</button><div className="connection-profile-heading"><div className="connection-friend-avatar">{member.avatarUrl ? <img src={member.avatarUrl} alt="" /> : Array.from(member.nickname)[0]}</div><div><h2 id="incoming-member-name">{member.nickname}</h2><p>{member.school} · {member.city}</p></div></div><p>{member.intro}</p>{member.skills.length ? <div className="map-member-tags">{member.skills.map((skill) => <span key={skill}>{skill}</span>)}</div> : null}<div className="connection-request-copy"><strong>对方的申请留言</strong><p>{item.request.message}</p></div>{error ? <p className="connection-error" role="alert">{error}</p> : null}<div className="connection-card-actions"><button type="button" disabled={busy} onClick={() => onAction("decline")}>婉拒</button><button type="button" disabled={busy} onClick={() => onAction("accept")}>接受</button></div></section></Modal>;
}
