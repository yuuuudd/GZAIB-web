...
const configPath = join(root, "wrangler.local.jsonc");
const readme = readFileSync(join(root, "README.md"), "utf8");
const viteConfig = readFileSync(join(root, "vite.config.ts"), "utf8");

test("local Demo migrations use a project-local Wrangler config with no remote resource id", () => {
  assert.ok(existsSync(configPath), "the explicit CLI-only Wrangler config must exist");
  ...
  assert.match(config, /"compatibility_date"\s*:\s*"2026-05-22"/);
  ...
  assert.match(viteConfig, /compatibility_date:\s*"2026-05-22"/);
...
test("README gives the exact local-only config-backed migration commands in order", () => {
  const core = "npx.cmd wrangler d1 execute site-creator-d1 --local --config wrangler.local.jsonc --file=drizzle/0000_builder_map_core.sql --persist-to=.wrangler/state";
  const connections = "npx.cmd wrangler d1 execute site-creator-d1 --local --config wrangler.local.jsonc --file=drizzle/0001_member_connections.sql --persist-to=.wrangler/state";
