...
function memoryRepository() {
  const ids = {
    ...
    profiles: new Set<string>(), visibility: new Set<string>(), contributions: new Set<string>(), audits: new Set<string>(),
  };
...
test("chunks every Demo seed insert below D1's bind-parameter budget", () => {
  for (const rows of [DEMO_SEED.schools, DEMO_SEED.users, DEMO_SEED.applications, DEMO_SEED.profiles, DEMO_SEED.visibility, DEMO_SEED.contributions]) {
