...
export async function GET(request: Request, context: Context) {
  let session; try { session = await requireRequestUserSession(request); } catch { return Response.json({ error: "请先登录有效账号" }, { status: 401, headers: { "Cache-Control": "private, no-store" } }); }
