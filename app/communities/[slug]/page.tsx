...
export default async function CommunityPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="community-shell"><header className="brand-header community-header"><Link className="brand-mark" href="/" aria-label="广州AI共创社首页"><Image src="/logo.png" alt="广州AI共创社" width={44} height={44} priority /><span>广州AI共创社</span></Link><nav className="brand-nav" aria-label="主导航"><a href="/#map">共建地图</a><Link className="brand-nav-active" href="/communities" aria-current="page">AI 社群</Link><Link href="/news">AI 资讯</Link><Link href="/events">活动赛事</Link></nav><Link className="brand-header-action" href="/apply">申请加入</Link></header><div className="community-page-content"><nav className="community-breadcrumb" aria-label="面包屑"><Link href="/communities">AI 社群</Link><span aria-hidden="true">/</span><strong>{community.name}</strong></nav><CommunityProfile community={community} isLoggedIn={Boolean(viewerId)} /></div></main>;
}
